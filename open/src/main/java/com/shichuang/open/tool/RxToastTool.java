...
public class RxToastTool {
    ...
    private static int yOffset = (int) (64 * Open.getInstance().getContext().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setGravity(final int gravity, final int xOffset, final int yOffset) {
        RxToastTool.gravity = gravity;
        RxToastTool.xOffset = xOffset;
        RxToastTool.yOffset = yOffset;
    }
    ...
    public static void setBgColor(@ColorInt final int backgroundColor) {
        RxToastTool.bgColor = backgroundColor;
    }
    ...
    public static void setBgResource(@DrawableRes final int bgResource) {
        RxToastTool.bgResource = bgResource;
    }
    ...
    private static void show(@StringRes final int resId, final int duration) {
        show(Open.getInstance().getContext().getResources().getText(resId).toString(), duration);
    }

    private static void show(@StringRes final int resId, final int duration, final Object... args) {
        show(String.format(Open.getInstance().getContext().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(final View view, final int duration) {
        HANDLER.post(new Runnable() {
            ...
            public void run() {
                cancel();
                sToast = new Toast(Open.getInstance().getContext());
                sToast.setView(view);
    ...
    private static View getView(@LayoutRes final int layoutId) {
        ...
        LayoutInflater inflate = (LayoutInflater) Open.getInstance().getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
